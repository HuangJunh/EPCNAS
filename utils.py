...
class Utils(object):
        ...
        for id, particle in enumerate(population):
            ...
            for number, dimen in enumerate(particle_a):
                ...
                for j, bit in enumerate(bin_code):
                    if int(bit) == 1:  # shortcut
                        if j == 0:
                            input_layers += 'conv_begin, '
                        elif 0 <= particle_a[j-1] <=15:
    ...
    def calc_in_out_channels(cls, particle_a, particle_c):
        ...
        # for i, val in enumerate(particle_c):
    ...
    def obtain_output_size(cls, particle_a, particle_c):
        ...
        for i in range(len(particle_a)):
            ...
            input_feature_sizes = []
            for j, bit in enumerate(bin_code):
                if int(bit) == 1:  # connected
                    input_feature_sizes.append(output_feature_sizes[j])
            input_feature_size = min(input_feature_sizes)
            if 0 <= particle_a[i] <= 15:
                output_feature_sizes.append(input_feature_size) # conv does not change size
            else:
                output_feature_size = int((input_feature_size-1)/2)+1   # pool - downsample size
    ...
    def generate_concat_code(cls, particle_a, particle_c, i, bin_code, forward_list):
        ...
        for k, [pos, input_feature_size] in enumerate(curr_input_feature_sizes):
            if input_feature_size == target_input_feature_size:  # at least one does not use sdconv, find it first
                if pos == -1:
        ...
        for pos, input_feature_size in curr_input_feature_sizes:
            if input_feature_size == target_input_feature_size:  # directly concatenate those do not use sdconv
                if pos == -1:  # from input node
                    tar = 'out_begin'
            ...
            else:  # input_feature_size > target_input_feature_size
                if pos == -1:  # from input node
    ...
    def generate_pytorch_file(cls, particle, curr_gen, id):
        ...
        for i, dimen in enumerate(particle_a):
            ...
            if 0 <= dimen <= 15:
                ...
                if conv_name not in conv_name_list:
                    pool_sta = [1 if 32 <= particle_a[k] <= 47 else 0 for k in range(0, i)]
                    num_pool = sum(pool_sta)
                    conv_name_list.append(conv_name)
                    conv = '%s = BasicBlock(in_planes=%d, planes=%d, kernel_size=%d, expansion_rate=%d, act_func="%s", drop_connect_rate=%.1f)' % (conv_name, in_channel, out_channel, kernel_sizes[num_pool], expansion_rates[num_pool],act_funcs[num_pool],drop_rates[num_pool])
                    conv_list.append(conv)

        conv_end1 = 'self.conv_end = nn.Conv2d(%d, %d, kernel_size=1, stride=1, bias=False)'%(channels_end, channels_end)
        ...
        for idx, val in enumerate(particle_c):
            ...
            if bin_code.count('1') > 1:
                ...
                for pos,input_feature_size in curr_input_feature_sizes:
                    if input_feature_size > target_input_feature_size:
                        if pos == -1:   #input node
                            sdconv = 'self.sdconv_begin_%d = nn.Conv2d(%d, %d, kernel_size=3, stride=%d, padding=1, bias=False, groups=%d)'%(idx, inchannels[0],inchannels[0],math.ceil(input_feature_size/target_input_feature_size),inchannels[0])
                        else:
                            sdconv = 'self.sdconv_%d_%d = nn.Conv2d(%d, %d, kernel_size=3, stride=%d, padding=1, bias=False, groups=%d)' % (pos, idx, outchannels[pos], outchannels[pos],
                            math.ceil(input_feature_size / target_input_feature_size),outchannels[pos])
                        conv_list.append(sdconv)
        # make sure the resolutions for different input features are identical
        curr_input_feature_sizes = [[indx, output_feature_sizes[indx+1]] for indx in layer_indices]
        target_input_feature_size = min([k for _,k in curr_input_feature_sizes])
        for pos, input_feature_size in curr_input_feature_sizes:
            if input_feature_size > target_input_feature_size:
                if pos == -1:  # input node
                    ...
                    pos, outchannels[pos], outchannels[pos], math.ceil(input_feature_size / target_input_feature_size),outchannels[pos])
                conv_list.append(sdconv)
        ...
        fully_layer_name2 = 'self.dropout = nn.Dropout(p=%.1f, inplace=True)' % (0.2)
